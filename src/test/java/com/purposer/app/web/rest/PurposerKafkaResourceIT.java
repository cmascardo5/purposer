...
import com.purposer.app.config.EmbeddedKafka;
import com.purposer.app.config.KafkaSseConsumer;
import com.purposer.app.config.KafkaSseProducer;
...
class PurposerKafkaResourceIT {
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/purposer-kafka/publish?message=value-produce")).andExpect(status().isOk());
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/purposer-kafka/register"))
        ...
        for (int i = 0; i < 100; i++) {
            ...
            if (content.contains("data:value-consume")) {
                restMockMvc.perform(get("/api/purposer-kafka/unregister"));
                return;
            }
        }
